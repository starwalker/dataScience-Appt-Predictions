...
data['PatientCancelPercentageLast12'] = [data.loc[i,'PatientCancelledAppointmentsLast12']/data.loc[i,'AppointmentsLast12'] for i in range(data.shape[0])]
data['ProviderCancelPercentageLast12'] = [data.loc[i,'ProviderCancelledAppointmentsLast12']/data.loc[i,'AppointmentsLast12'] for i in range(data.shape[0])]
...
col_dict = {'cat_cols': ['AppointmentTypeCategoryDescription','AppointmentTypeDescription','PatientMarketingRegion','PatientSex',
                         'PatientStateCode','PayorFinancialClass','SchedulingProviderReportingSpecialty', 'ClinicalDepartment','DepartmentSpecialty'],
            'zero_imputer_cols':['AppointmentHourOfDay','AppointmentMonth','ScheduledLagDays','ScheduledMinutes',
                                'CopayDue','PatientAge','ReferralFlag','CompletedAppointmentPercentageLast12',
                                'PatientCancelPercentageLast12','ProviderCancelPercentageLast12','FromSCFlag',
                                 'AppointmentDayOfWeekNumber','AppointmentDayOfMonth','ACOPatientPopulationFlag',
                                'AppointmentsLast12',
...
trans.update_params({'text_cols': {'max_features': 800, 'min_freq': 0.003, 'ngram_range': (2, 3), 'stop_words': stops},
                     'cat_cols': {'min_freq': .001}})
trans.fit(data)
